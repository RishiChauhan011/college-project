    ...
    <aside className="hidden lg:flex flex-col h-screen fixed left-0 top-0 pt-24 pb-8 px-4 docked w-64 bg-surface dark:bg-inverse-surface shadow-[4px_0px_10px_rgba(163,177,198,0.3)] shadow-lg z-40">
      <div className="mb-8 px-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-data-sm font-data-sm text-on-surface-variant">{user?.profile?.preferred_field || (user?.role === 'admin' ? 'Administrator' : 'Explorer')}</p>
        </div>
      </div>
      ...
      <div className="mt-auto flex flex-col gap-2 pt-4 border-t border-surface-variant">
        <button className="w-full bg-surface-bright text-primary border border-primary/20 px-4 py-2 rounded-lg font-data-sm text-data-sm elevation-1 hover:bg-primary-fixed transition-colors mb-4 flex items-center justify-center gap-2">
          <span className="material-symbols-outlined text-[18px]">support_agent</span> Career Concierge
        </button>
