...
const AdminDashboard = () => {
  ...
  const [rangeVal, setRangeVal] = useState(7);
  ...
  // Process growth data to trim leading zero days if range is extended
  const processGrowthData = () => {
    if (!growthData || growthData.length === 0) return [];
    if (rangeVal <= 7) return growthData;
    const firstNonZeroIdx = growthData.findIndex((d) => d.registrations > 0);
    if (firstNonZeroIdx > 0 && firstNonZeroIdx < growthData.length - 3) {
      return growthData.slice(Math.max(0, firstNonZeroIdx - 1));
    }
    return growthData;
  };

  const chartData = processGrowthData();
  const nonZeroDaysCount = chartData.filter((d) => d.registrations > 0).length;
  ...
  return (
    <AdminLayout>
      ...
      <div className="bg-surface-container-lowest rounded-xl p-6 shadow-[2px_2px_6px_rgba(163,177,198,0.4),-2px_-2px_6px_rgba(255,255,255,0.9)] flex flex-col mb-8">
        ...
        <div className="flex-1 bg-surface-bright rounded-lg shadow-[inset_2px_2px_5px_rgba(163,177,198,0.4),inset_-2px_-2px_5px_rgba(255,255,255,0.7)] p-4 relative min-h-[220px]">
          {nonZeroDaysCount < 3 && chartData.length > 0 && (
            <div className="mb-3 text-[11px] font-data-sm text-secondary bg-surface/90 px-3 py-1.5 rounded-lg border border-outline-variant/30 flex items-center justify-between shadow-sm">
              <span className="flex items-center gap-1.5">
                <span className="material-symbols-outlined text-[15px] text-primary">info</span>
                Not enough registration history yet — showing available trend trajectory.
              </span>
            </div>
          )}
          {chartData.length > 0 ? (
      ...
      {/* Bottom Section: Recent Activity Timeline (Full Width) */}
